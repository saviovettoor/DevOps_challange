...
4. Production grade setup.
    ...
    1. Setting up the cluster in cloud(aws/gcp/azure) with HA.
    2. Enableing both Horizontal and vertical scale of app (Based on traffic/CPU/Memory).
    3. In one click provisioning entire cluster setup (using terraform or ansible).
    4. Monitoing tha application and cluster (prometheus/nagios )
    5. Enhaceing the deployment using Istio by canary deployment or using Blue/Green Deployment[link](https://github.com/saviovettoor/risk.ident_challange/blob/master/BlueGreenDeploymentUsingIstio/README.md#bluegreen-deployment-using-istio)  
```
